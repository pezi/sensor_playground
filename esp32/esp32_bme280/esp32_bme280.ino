/*
 * Sensor Tester Sensor Node — ESP32 + BME280
 *
 * Implements the Sensor Tester Sensor Interface (see docs/sensor.md).
 * Reads a BME280 (temperature, humidity, pressure) via I2C.
 *
 ...
 *
 * Required Libraries:
 *   ArduinoJson, Wire, Adafruit BME280 Library (+ Adafruit Unified Sensor),
...
// --- Sensor ---
Adafruit_BME280 bme;
const char* SENSOR_NAME = "BME280";

// Many BME280 breakout boards use 0x76; some use 0x77.
...
void setup() {
  ...
  if (!bme.begin(BME280_ADDR)) {
    Serial.println("Error: BME280 not found! (try address 0x77)");
    while (1) delay(1000);
  }
