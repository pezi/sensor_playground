 ...
 *
 * Required Libraries:
 *   ArduinoJson, Wire, Adafruit MPU6050 (+ Adafruit Unified Sensor),
...
const char* SENSOR_NAME = "MPU6050";

// Standard gravity, used to convert the driver's m/s^2 readings into g.
...
void setup() {
  ...
  Wire.begin();
  if (!mpu.begin()) {
    Serial.println("Error: MPU6050 not found! (try address 0x69)");
    while (1) delay(1000);
  }
...
String buildSensorJson(bool shortKeys) {
  ...
  if (mpu.getEvent(&accel, &gyro, &temp)) {
    // The driver reports acceleration in m/s^2; convert to g.
    float ax = accel.acceleration.x / GRAVITY;
    float ay = accel.acceleration.y / GRAVITY;
    float az = accel.acceleration.z / GRAVITY;
