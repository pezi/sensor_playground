 ...
 *   (No sensor library — the photo-resistor is read with analogRead.)
 */
...
// --- Sensor ---
const char* SENSOR_NAME = "LIGHT";

// ADC pin the Grove Light Sensor signal (yellow) wire is connected to.
...
// Reads the current brightness (0-4095).
int readLight() {
...
String buildSensorJson(bool shortKeys) {
  ...
  // Same short/long key ("light") on both the discovery and REST payloads.
  doc["light"] = readLight();
