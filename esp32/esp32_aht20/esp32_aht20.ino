/*
 * Sensor Tester Sensor Node — ESP32 + AHT10/AHT20
 *
 * Implements the Sensor Tester Sensor Interface (see docs/sensor.md).
 * Reads an ASAIR AHT10 or AHT20 (temperature, humidity) via I2C; both
 * chips share the fixed address 0x38 and the same measurement protocol,
 * so this sketch works with either one unchanged.
 *
 ...
 *
 * Required Libraries:
 *   ArduinoJson, Wire, Adafruit AHTX0,
...
// --- Sensor ---
Adafruit_AHTX0 aht;
// Set to "AHT10" if that is the chip on your board (same protocol).
const char* SENSOR_NAME = "AHT20";
...
void setup() {
  ...
  Wire.begin();
  if (!aht.begin()) {
    Serial.println("Error: AHT10/AHT20 not found at 0x38!");
    while (1) delay(1000);
  }
...
String buildSensorJson(bool shortKeys) {
  ...
  if (aht.getEvent(&humidity, &temperature)) {
    if (shortKeys) {
      doc["temp"] = temperature.temperature;
