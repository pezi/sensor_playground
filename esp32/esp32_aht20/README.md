# ESP32 AHT10/AHT20 Sensor Node for Sensor Tester

This Arduino project implements the Sensor Tester
[Sensor Interface](../../../docs/sensor.md) on an ESP32 with an ASAIR
AHT10 or AHT20 I2C sensor (temperature, humidity).

> The AHT20 (±0.3 °C / ±2 %RH typical) is the successor of the AHT10;
> both share the fixed I2C address `0x38` and the same measurement
> protocol, so this sketch works with either chip unchanged. Neither has
> a pressure or air-quality output. Set `SENSOR_NAME` in the sketch to
> `"AHT10"` if that is the chip on your board so the app shows the right
> name.
...
- **AHT10** or **AHT20** module (temperature, humidity)
...
> The I2C address is fixed at `0x38` on both chips. Note the AHT10 does
> not tolerate other devices on the same I2C bus; give it a dedicated bus
> or use an AHT20.
...
3. **Libraries** (install via Library Manager):
   ...
   - `Adafruit AHTX0` (drives both AHT10 and AHT20)
...
```json
{"sensor":"AHT20","host":"ESP32","temperature":22.3,"humidity":46.9}
```
