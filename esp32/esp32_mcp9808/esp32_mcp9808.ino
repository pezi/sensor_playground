/*
 * Sensor Tester Sensor Node — ESP32 + MCP9808
 *
 * Implements the Sensor Tester Sensor Interface (see docs/sensor.md).
 * Reads an MCP9808 high-accuracy temperature sensor via I2C.
 *
 ...
 *
 * Required Libraries:
 *   ArduinoJson, Wire, Adafruit MCP9808 Library (+ Adafruit Unified Sensor),
...
const char* SENSOR_NAME = "MCP9808";

// Default I2C address; jumper-selectable in the range 0x18–0x1F.
...
void setup() {
  ...
  if (!mcp.begin(MCP9808_ADDR)) {
    Serial.println("Error: MCP9808 not found! (check address 0x18-0x1F)");
    while (1) delay(1000);
  }
  mcp.setResolution(3);  // 0.0625 °C, ~250 ms conversion (highest precision)
...
String buildSensorJson(bool shortKeys) {
  ...
  float temperature = mcp.readTempC();  // °C
