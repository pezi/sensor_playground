/*
 * Sensor Tester Sensor Node — ESP32 + TCS34725 (Grove I2C Color Sensor)
 *
 * Implements the Sensor Tester Sensor Interface (see docs/sensor.md).
 * Reads a Grove I2C Color Sensor V2 (TCS34725: RGB color, color temperature,
 * illuminance) via I2C.
 *
 ...
 *
 * The RGB channels are normalized against the clear channel to 0–255 so the
 * app can show the measured color directly; color temperature (Kelvin) and
 * illuminance (lux) are computed by the Adafruit driver.
 *
 * Required Libraries:
 *   ArduinoJson, Wire, Adafruit TCS34725,
...
Adafruit_TCS34725 tcs = Adafruit_TCS34725(TCS34725_INTEGRATIONTIME_154MS,
                                          TCS34725_GAIN_4X);
const char* SENSOR_NAME = "TCS34725";
...
String buildSensorJson(bool shortKeys) {
  ...
  if (c > 0) {
    ...
    // Normalize the channels against clear so the app can show the color.
    int red   = (int)((float)r / c * 255.0);
    int green = (int)((float)g / c * 255.0);
    int blue  = (int)((float)b / c * 255.0);
    if (red > 255) red = 255;
    ...
    if (shortKeys) {
      ...
      doc["colorTemperature"] = colorTemp;
      doc["lux"]              = lux;
      doc["red"]              = red;
      doc["green"]            = green;
      doc["blue"]             = blue;
