/*
 * Sensor Tester Sensor Node — ESP32 + SI1145 (Grove Sunlight Sensor)
 *
 * Implements the Sensor Tester Sensor Interface (see docs/sensor.md).
 * Reads a Grove Sunlight Sensor (SI1145: visible light, infrared, UV index)
 * via I2C.
 *
 ...
 *
 * Visible and IR are raw sensor counts (the SI1145 is not lux-calibrated);
 * the UV index is computed by the chip from the visible/IR photodiodes.
 *
 * Required Libraries:
 *   ArduinoJson, Wire, Grove Sunlight Sensor (Seeed Studio, SI114X),
...
String buildSensorJson(bool shortKeys) {
  ...
  // The library reports the UV index multiplied by 100.
  float uvIndex    = (float)si1145.ReadUV() / 100.0;
