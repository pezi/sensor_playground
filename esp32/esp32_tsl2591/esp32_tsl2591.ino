 ...
 *
 * The sensor has two photodiodes: channel 0 is broadband (visible + IR),
 * channel 1 is infrared only. Visible light is the difference of the two,
 * and the Adafruit driver derives lux from both channels together with the
 * configured gain and integration time — which is why those two must be set
 * on the driver and not just on the chip.
 *
 ...
 *
 * Required Libraries:
 *   ArduinoJson, Wire, Adafruit TSL2591 Library, Adafruit Unified Sensor,
...
const char* SENSOR_NAME = "TSL2591";

// Gain and integration time. The pair sets both the sensitivity and the
// saturation point: 25x/300ms suits normal indoor light. Use GAIN_LOW (1x)
// with 100 ms outdoors in daylight, or GAIN_MAX (9876x) for moonlight —
// autoGain() below moves between them when a reading pins at either end.
const tsl2591Gain_t DEFAULT_GAIN = TSL2591_GAIN_MED;
const tsl2591IntegrationTime_t DEFAULT_INTEGRATION = TSL2591_INTEGRATIONTIME_300MS;

// Whether to follow the light level by shifting gain (see autoGain()).
const bool AUTO_GAIN = true;

// Channel-0 counts at which the current gain is judged too high or too low.
// The ADC is 16-bit, so a saturated channel reads 0xFFFF and lux is invalid.
const uint16_t SATURATION_COUNTS = 0xFFFF;
const uint16_t TOO_DARK_COUNTS   = 100;

tsl2591Gain_t gCurrentGain = DEFAULT_GAIN;
...
// already in flight, so the *next* reading is the one that benefits. Jumping
// straight to the extreme instead would make the value oscillate whenever the
// light sits near a threshold.
void autoGain(uint16_t broadband) {
  if (!AUTO_GAIN) return;

  tsl2591Gain_t next = gCurrentGain;
  if (broadband >= SATURATION_COUNTS) {
    switch (gCurrentGain) {
      case TSL2591_GAIN_MAX:  next = TSL2591_GAIN_HIGH; break;
      case TSL2591_GAIN_HIGH: next = TSL2591_GAIN_MED;  break;
      case TSL2591_GAIN_MED:  next = TSL2591_GAIN_LOW;  break;
      default: break;  // already at 1x — the light is simply too bright
    }
  } else if (broadband <= TOO_DARK_COUNTS) {
    switch (gCurrentGain) {
      case TSL2591_GAIN_LOW:  next = TSL2591_GAIN_MED;  break;
      case TSL2591_GAIN_MED:  next = TSL2591_GAIN_HIGH; break;
      case TSL2591_GAIN_HIGH: next = TSL2591_GAIN_MAX;  break;
      default: break;  // already at 9876x — the scene is simply that dark
    }
  }

  if (next != gCurrentGain) {
    gCurrentGain = next;
    tsl.setGain(gCurrentGain);
  }
}
...
void setup() {
  ...
  tsl.setGain(DEFAULT_GAIN);
  tsl.setTiming(DEFAULT_INTEGRATION);
  gCurrentGain = DEFAULT_GAIN;
...
String buildSensorJson(bool shortKeys) {
  ...
  // One read returns both channels packed into 32 bits: IR high, broadband
  // low. Reading them separately would sample two different integrations.
  uint32_t luminosity = tsl.getFullLuminosity();
  uint16_t ir         = luminosity >> 16;
  uint16_t broadband  = luminosity & 0xFFFF;

  // Broadband minus IR, floored: sensor noise can put IR marginally above
  // broadband in the dark, and a negative "visible light" is meaningless.
  uint16_t visible = broadband > ir ? broadband - ir : 0;
  float lux        = tsl.calculateLux(broadband, ir);

  // The driver returns a negative lux when a channel saturated. Report the
  // raw counts anyway — they still show the app that it is very bright —
  // but omit the lux rather than publish a value known to be wrong.
  if (shortKeys) {
    doc["vis"] = visible;
    doc["ir"]  = ir;
