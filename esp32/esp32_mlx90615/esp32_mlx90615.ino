 ...
 *
 * The MLX90615 is driven directly over Wire (SMBus word reads from RAM
 * registers 0x26 ambient / 0x27 object; value * 0.02 K - 273.15) — no
 * sensor library is needed.
 *
 * Required Libraries:
 *   ArduinoJson, Wire,
...
// --- Sensor ---
const char* SENSOR_NAME = "MLX90615";

const uint8_t MLX90615_ADDR        = 0x5B;
const uint8_t MLX90615_REG_AMBIENT = 0x26;
const uint8_t MLX90615_REG_OBJECT  = 0x27;
...
// Read one temperature RAM register; returns NAN on bus errors.
float readMlxTemperature(uint8_t reg) {
  Wire.beginTransmission(MLX90615_ADDR);
  Wire.write(reg);
  if (Wire.endTransmission(false) != 0) return NAN;

  if (Wire.requestFrom((int)MLX90615_ADDR, 3) < 3) return NAN;
  uint8_t lsb = Wire.read();
  uint8_t msb = Wire.read();
  Wire.read();  // PEC byte, not verified
...
void transportLoop() {
  ...
  if (deviceConnected && authed && millis() - lastNotifyMs >= 1000) {
    lastNotifyMs = millis();
    String json = buildSensorJsonForTransport(false);
    if (json.indexOf("objectTemperature") >= 0) {
...
void setup() {
  ...
  if (isnan(readMlxTemperature(MLX90615_REG_AMBIENT))) {
    Serial.println("Error: MLX90615 not found!");
    while (1) delay(1000);
  }
...
String buildSensorJson(bool shortKeys) {
  ...
  float ambient = readMlxTemperature(MLX90615_REG_AMBIENT);
  float object  = readMlxTemperature(MLX90615_REG_OBJECT);
  ...
  if (!isnan(ambient) && !isnan(object)) {
    if (shortKeys) {
      ...
      doc["temperature"]       = ambient;
      doc["objectTemperature"] = object;
    }
  }
