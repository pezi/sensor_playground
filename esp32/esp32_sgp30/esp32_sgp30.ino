/*
 * Sensor Tester Sensor Node — ESP32 + SGP30
 *
 * Implements the Sensor Tester Sensor Interface (see docs/sensor.md).
 * Reads an SGP30 air-quality sensor (eCO2, TVOC) via I2C.
 *
 ...
 *
 * The JSON payload is identical on both transports. Regardless of transport,
 * the sensor is sampled at ~1 Hz to maintain its dynamic baseline; the first
 * ~15 s after power-up it reports eCO2=400, TVOC=0.
 *
 * Required Libraries:
 *   ArduinoJson, Wire, Adafruit SGP30 Sensor Library (+ Adafruit Unified
 *   Sensor), and for TRANSPORT_WIFI: WiFi, WiFiUdp.
...
// --- Sensor ---
Adafruit_SGP30 sgp;
const char* SENSOR_NAME = "SGP30";

// Latest measurement, refreshed at ~1 Hz from loop() so the SGP30 keeps its
// dynamic baseline regardless of how often clients poll.
uint16_t latestEco2 = 400;
...
void updateMeasurement() {
  if (millis() - lastMeasureMs < 1000) return;
...
void transportLoop() {
  // Push the latest cached reading once per second to subscribed,
  // authorized clients.
