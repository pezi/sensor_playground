/*
 * Sensor Tester Sensor Node — ESP32 + Grove IMU 10DOF (MPU9250 + BMP280)
 *
 * Implements the Sensor Tester Sensor Interface (see docs/sensor.md).
 * Reads a Grove IMU 10DOF via I2C: the MPU9250 (accelerometer, gyroscope,
 * magnetometer) is fused into roll / pitch / heading angles plus the total
 * acceleration magnitude (g-force); the BMP280 adds temperature and
 * barometric pressure.
 *
 ...
 *
 * The MPU9250 sensor fusion needs a continuous update loop; the fused values
 * are cached and streamed to connected clients, regardless of transport.
 *
 * Required Libraries:
 *   ArduinoJson, Wire,
...
MPU9250 mpu;
Adafruit_BMP280 bmp;
const char* SENSOR_NAME = "IMU10DOF";

// Latest fused IMU values, refreshed continuously from loop().
float latestRoll = 0;
float latestPitch = 0;
float latestHeading = 0;
float latestGForce = 1.0;

String buildSensorJson(bool shortKeys);

// Refresh the cached fusion values when the MPU9250 has new data.
void updateImu() {
  if (!mpu.update()) return;
  latestRoll = mpu.getRoll();
  latestPitch = mpu.getPitch();
  float yaw = mpu.getYaw();
  latestHeading = yaw < 0 ? yaw + 360.0 : yaw;
  float ax = mpu.getAccX();
  float ay = mpu.getAccY();
  float az = mpu.getAccZ();
  latestGForce = sqrt(ax * ax + ay * ay + az * az);
}
...
void transportLoop() {
  ...
  // motion sensors instead of polling them, so there is no REST endpoint.
...
void setup() {
  ...
  // Grove IMU 10DOF v2.0 carries a BMP280 at I2C address 0x77.
  if (!bmp.begin(0x77)) {
...
String buildSensorJson(bool shortKeys) {
  ...
  doc["roll"]    = latestRoll;
  doc["pitch"]   = latestPitch;
  doc["heading"] = latestHeading;
  doc["gforce"]  = latestGForce;
