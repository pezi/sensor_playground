# SHT31 Sensor Node for Sensor Tester (Python)

This Python script implements the Sensor Tester sensor interface on
single-board computers with a SHT31 I2C sensor (temperature, humidity).
...
`../../esp32/esp32_sht31/` and supports the same two transports (Wi-Fi
and BLE).
...
Create `/etc/systemd/system/sensor-tester-sht31.service`:

```ini
[Unit]
Description=Sensor Tester SHT31 Node
...
Type=simple
User=pi
WorkingDirectory=/home/pi/sensor-sht31
ExecStart=/home/pi/sensor-sht31/venv/bin/python3 sensor_node.py
Restart=on-failure
RestartSec=5
...
sudo systemctl enable sensor-tester-sht31
sudo systemctl start sensor-tester-sht31
```
