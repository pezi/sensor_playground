# MLX90615 Infrared Thermometer Node for Sensor Tester (Python)

This Python script implements the Sensor Tester sensor interface on
single-board computers with a Grove Digital Infrared Temperature Sensor
(MLX90615). It measures the **object temperature** of whatever is in the
sensor's field of view — without contact — plus the **ambient temperature**
of the sensor itself. Over Wi-Fi the Sensor Tester app discovers this node
via UDP broadcast (port 9133) and polls it for data over HTTPS (port 9132,
...
`../../esp32/esp32_mlx90615/` and supports the same two transports (Wi-Fi
and BLE).

> The MLX90615 is driven directly via smbus2 (SMBus word reads from RAM
> registers `0x26` ambient / `0x27` object; temperature = raw × 0.02 K −
> 273.15) — no extra driver package needed.
...
Create `/etc/systemd/system/sensor-tester-mlx90615.service`:

```ini
[Unit]
Description=Sensor Tester MLX90615 Node
...
Type=simple
User=pi
WorkingDirectory=/home/pi/sensor-mlx90615
ExecStart=/home/pi/sensor-mlx90615/venv/bin/python3 sensor_node.py
Restart=on-failure
RestartSec=5
...
sudo systemctl enable sensor-tester-mlx90615
sudo systemctl start sensor-tester-mlx90615
```
