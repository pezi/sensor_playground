# SI1145 Sensor Node for Sensor Tester (Python)

This Python script implements the Sensor Tester sensor interface on
single-board computers with a SI1145 I2C sensor (visible light, infrared, UV index).
...
`../../esp32/esp32_si1145/` and supports the same two transports (Wi-Fi
and BLE).
...
The `SI1145` library pulls in `Adafruit-GPIO`/`spidev`, which build C
extensions from source. Unlike the other (pure `smbus2`) sensor nodes, this
one needs the Python development headers and a compiler installed first,
otherwise `pip install` fails with `Python.h: No such file or directory`:
...
# Install dependencies (wheel first, so spidev builds without the slow
# legacy setup.py path)
pip install wheel
...
Create `/etc/systemd/system/sensor-tester-si1145.service`:

```ini
[Unit]
Description=Sensor Tester SI1145 Node
...
Type=simple
User=pi
WorkingDirectory=/home/pi/sensor-si1145
ExecStart=/home/pi/sensor-si1145/venv/bin/python3 sensor_node.py
Restart=on-failure
RestartSec=5
...
sudo systemctl enable sensor-tester-si1145
sudo systemctl start sensor-tester-si1145
```
