# SGP30 Sensor Node for Sensor Tester (Python)

This Python script implements the Sensor Tester sensor interface on
single-board computers with a SGP30 I2C sensor (eCO2, TVOC).
...
`../../esp32/esp32_sgp30/` and supports the same two transports (Wi-Fi
and BLE).
...
Create `/etc/systemd/system/sensor-tester-sgp30.service`:

```ini
[Unit]
Description=Sensor Tester SGP30 Node
...
Type=simple
User=pi
WorkingDirectory=/home/pi/sensor-sgp30
ExecStart=/home/pi/sensor-sgp30/venv/bin/python3 sensor_node.py
Restart=on-failure
RestartSec=5
...
sudo systemctl enable sensor-tester-sgp30
sudo systemctl start sensor-tester-sgp30
```
...
The SGP30 needs a ~15 second warm-up after start; the script blocks in
`start_measurement()` until the sensor reports valid values.
