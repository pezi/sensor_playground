# MMA7660 Accelerometer Node for Sensor Tester (Python)
...
total acceleration magnitude (g-force). Over Wi-Fi the Sensor Tester app
...
handshake) — the app streams accelerometers rather than polling them; over
BLE the node advertises the Sensor Tester GATT service instead.

It is the Python/SoC counterpart of the ESP32 sketch in
`../../esp32/esp32_mma7660/` and supports the same two transports (Wi-Fi
and BLE).

> The MMA7660 is driven directly via smbus2 (6-bit two's-complement axes,
> 21.33 counts per g) — no extra driver package needed.
...
Create `/etc/systemd/system/sensor-tester-mma7660.service`:

```ini
[Unit]
Description=Sensor Tester MMA7660 Node
...
Type=simple
User=pi
WorkingDirectory=/home/pi/sensor-mma7660
ExecStart=/home/pi/sensor-mma7660/venv/bin/python3 sensor_node.py
Restart=on-failure
RestartSec=5
...
sudo systemctl enable sensor-tester-mma7660
sudo systemctl start sensor-tester-mma7660
```
