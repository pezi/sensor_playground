"""
Sensor Tester Sensor Node — SHT41 (Python)

Implements the Sensor Tester Sensor Interface on single-board computers
(Raspberry Pi & co.) with a SHT41 I2C sensor (temperature, humidity).
...
class SHT41Sensor:
    """Reads temperature and humidity from an SHT41 (single-shot mode)."""
    ...
    def read(self):
        ...
        # High-precision single-shot measurement (command 0xFD, ~8 ms).
        self.bus.i2c_rdwr(self._i2c_msg.write(self.address, [0xFD]))
        time.sleep(0.01)
...
class EmulatedSHT41Sensor(SHT41Sensor):
    """Generates plausible SHT41 readings without hardware.
...
def main():
    ...
    if config.get("emulation", False):
        print("Emulation mode: generating SHT41 readings without hardware")
        _sensor = EmulatedSHT41Sensor()
    else:
        print(f"Initializing SHT41 sensor on /dev/i2c-{i2c_bus}...")
        _sensor = SHT41Sensor(i2c_bus=i2c_bus)
