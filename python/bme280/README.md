# BME280 Sensor Node for Sensor Tester (Python)

This Python script implements the Sensor Tester sensor interface on
single-board computers with a BME280 I2C sensor (temperature, humidity, pressure).
...
`../../esp32/esp32_bme280/` and supports the same two transports (Wi-Fi
and BLE).
...
Create `/etc/systemd/system/sensor-tester-bme280.service`:

```ini
[Unit]
Description=Sensor Tester BME280 Node
...
Type=simple
User=pi
WorkingDirectory=/home/pi/sensor-bme280
ExecStart=/home/pi/sensor-bme280/venv/bin/python3 sensor_node.py
Restart=on-failure
RestartSec=5
...
sudo systemctl enable sensor-tester-bme280
sudo systemctl start sensor-tester-bme280
```
