# AHT10/AHT20 Sensor Node for Sensor Tester (Python)

This Python script implements the Sensor Tester sensor interface on
single-board computers with an ASAIR AHT10 or AHT20 I2C sensor
(temperature, humidity). Over Wi-Fi the Sensor Tester app discovers this
node via UDP broadcast (port 9133) and polls it for data over HTTPS
(port 9132, `X-Api-Key` header); over BLE the node advertises the Sensor
Tester GATT service instead.

It is the Python/SoC counterpart of the ESP32 sketch in
`../../esp32/esp32_aht20/` and supports the same two transports (Wi-Fi
and BLE).
...
Create `/etc/systemd/system/sensor-tester-aht20.service`:

```ini
[Unit]
Description=Sensor Tester AHT20 Node
...
Type=simple
User=pi
WorkingDirectory=/home/pi/sensor-aht20
ExecStart=/home/pi/sensor-aht20/venv/bin/python3 sensor_node.py
Restart=on-failure
RestartSec=5
...
sudo systemctl enable sensor-tester-aht20
sudo systemctl start sensor-tester-aht20
```
