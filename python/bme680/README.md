# BME680 Sensor Node for Sensor Tester (Python)

This Python script implements the Sensor Tester sensor interface on
single-board computers with a BME680 I2C sensor (temperature, humidity, pressure, IAQ).
...
`../../esp32/esp32_bme680/` and supports the same two transports (Wi-Fi
and BLE).
...
Create `/etc/systemd/system/sensor-tester-bme680.service`:

```ini
[Unit]
Description=Sensor Tester BME680 Node
...
Type=simple
User=pi
WorkingDirectory=/home/pi/sensor-bme680
ExecStart=/home/pi/sensor-bme680/venv/bin/python3 sensor_node.py
Restart=on-failure
RestartSec=5
...
sudo systemctl enable sensor-tester-bme680
sudo systemctl start sensor-tester-bme680
```
